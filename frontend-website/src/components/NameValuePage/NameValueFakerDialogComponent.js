...
import nameValueFakerFactory from "./nameValueFakerFactory";
...
const NameValueFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = nameValueFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("nameValue").create(elem));
...
export default connect(null, mapDispatch)(NameValueFakerDialogComponent);
