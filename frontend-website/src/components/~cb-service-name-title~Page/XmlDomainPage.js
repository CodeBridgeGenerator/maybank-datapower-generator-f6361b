...
import XmlLayout from "../Layouts/XmlLayout";
import { connect } from "react-redux";
import ~cb-service-name-title~Page from "./~cb-service-name-singular-title~Page";

const ~cb-service-name-singular-title~XmlPage = (props) => {
  return (
    <XmlLayout>
          ...
          >
            <div className="flex align-items-center text-white">
              <p className="text-4xl text-white">
                {props.selectedXml?.name + " > "} ~cb-service-label~
              </p>
            </div>
          </div>
...
export default connect(mapState, mapDispatch)(~cb-service-name-singular-title~XmlPage);