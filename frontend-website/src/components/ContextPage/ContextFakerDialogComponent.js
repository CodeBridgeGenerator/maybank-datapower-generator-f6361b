...
import contextFakerFactory from "./contextFakerFactory";
...
const ContextFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contextFakerFactory(fakerCount,domainIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("context").create(elem));
...
export default connect(null, mapDispatch)(ContextFakerDialogComponent);
