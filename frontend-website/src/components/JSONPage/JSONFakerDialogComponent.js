...
import jSONFakerFactory from "./jSONFakerFactory";
...
const JSONFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = jSONFakerFactory(fakerCount,domainIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jSON").create(elem));
...
export default connect(null, mapDispatch)(JSONFakerDialogComponent);
