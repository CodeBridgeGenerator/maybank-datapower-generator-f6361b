...
import domainsFakerFactory from "./domainsFakerFactory";
...
const DomainsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = domainsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("domains").create(elem));
...
export default connect(null, mapDispatch)(DomainsFakerDialogComponent);
