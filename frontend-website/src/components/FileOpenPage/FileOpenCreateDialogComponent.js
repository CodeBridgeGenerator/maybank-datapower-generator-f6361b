...
const methodOptions = methodArray.map((x,i) => ({ name: x, value: i }));
...
const FileOpenCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("fileOpen").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info fileOpen updated successfully" });
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="fileOpen-create-dialog-component">
            <div>
                <p className="m-0">domainId:</p>
            ...
            <div>
                ...
                <Dropdown value={_entity?.method} optionLabel="name" optionValue="value" options={methodOptions} onChange={(e) => setValByKey("method", e.value)} />
            </div>
...
export default connect(mapState, mapDispatch)(FileOpenCreateDialogComponent);
