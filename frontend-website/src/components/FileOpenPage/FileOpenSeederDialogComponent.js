...
import seederData from "./fileOpen.seeder.json";
...
const FileOpenSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("fileOpen").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(FileOpenSeederDialogComponent);
