...
import fileOpenFakerFactory from "./fileOpenFakerFactory";
...
const FileOpenFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fileOpenFakerFactory(fakerCount,domainIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fileOpen").create(elem));
...
export default connect(null, mapDispatch)(FileOpenFakerDialogComponent);
