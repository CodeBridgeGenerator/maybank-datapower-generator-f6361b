...
import xmlsFakerFactory from "./xmlsFakerFactory";
...
const XmlsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = xmlsFakerFactory(fakerCount,domainIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("xmls").create(elem));
...
export default connect(null, mapDispatch)(XmlsFakerDialogComponent);
