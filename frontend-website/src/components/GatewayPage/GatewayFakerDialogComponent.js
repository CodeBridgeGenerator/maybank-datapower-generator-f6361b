...
import gatewayFakerFactory from "./gatewayFakerFactory";
...
const GatewayFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = gatewayFakerFactory(fakerCount,domainIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("gateway").create(elem));
...
export default connect(null, mapDispatch)(GatewayFakerDialogComponent);
