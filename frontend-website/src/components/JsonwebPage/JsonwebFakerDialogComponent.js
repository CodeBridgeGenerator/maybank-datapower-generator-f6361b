...
import jsonwebFakerFactory from "./jsonwebFakerFactory";
...
const JsonwebFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = jsonwebFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("jsonweb").create(elem));
...
export default connect(null, mapDispatch)(JsonwebFakerDialogComponent);
