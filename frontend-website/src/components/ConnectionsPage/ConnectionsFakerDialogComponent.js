...
import connectionsFakerFactory from "./connectionsFakerFactory";
...
const ConnectionsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [bbRoutingIdIds, setbbRoutingIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            .service("bbrouting")
            .find({ query: { $select: ["_id","domainId"] } })
            .then((data) => {
              setbbRoutingIdIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = connectionsFakerFactory(fakerCount,bbRoutingIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("connections").create(elem));
...
export default connect(null, mapDispatch)(ConnectionsFakerDialogComponent);
