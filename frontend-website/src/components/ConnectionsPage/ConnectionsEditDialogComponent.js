...
const ConnectionsCreateDialogComponent = (props) => {
     ...
     useEffect(() => {
                    ...
                    client
                        ...
                        .then((res) => {
                            ...
                            setbbRoutingId(res.data.map((e) => ({ name: e['domainId'], value: e._id })));
                        })
                        .catch((error) => {
                            console.log({ error });
    ...
    const onSave = async () => {
        ...
        await client.service("connections").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info connections updated successfully" });
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="connections-edit-dialog-component">
                <div>
                <p className="m-0">B2B Routing Connection:</p>
                <MultiSelect value={_entity?.bbRoutingId?._id} options={bbRoutingIdOptions} onChange={(e) => setValByKey("bbRoutingId", e.value)} />
            </div>
            <div>
