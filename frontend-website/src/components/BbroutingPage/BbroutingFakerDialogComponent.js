...
import bbroutingFakerFactory from "./bbroutingFakerFactory";
...
const BbroutingFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    
                const [domainIdIds, setdomainIdIds] = useState();
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((data) => {
              setdomainIdIds(data.data);
            });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = bbroutingFakerFactory(fakerCount,domainIdIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("bbrouting").create(elem));
...
export default connect(null, mapDispatch)(BbroutingFakerDialogComponent);
